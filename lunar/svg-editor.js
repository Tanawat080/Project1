			...
			(function() {
				...
				var box = $('#dialog_box'),
					...
					dbox = function(type, msg, callback, defaultVal, opts, changeCb, checkbox) {
						...
						if (type === 'prompt') {
							//ctrl = $('<input type="file">').prependTo(btn_holder);
								
							//$('#tool_image').show().prepend(ctrl);		
							//ctrl.val(defaultVal || '');
...
'					<defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
			...
			setTimeout(function() {
				svgCanvas.embedImage('images/wave.png', function(datauri) {
					if (!datauri) {
						// Disable option
		...
		//			{sel: '#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
			...
			if (window.FileReader) {
				...
				$('#tool_import').show().prepend(imgImport);
					...
					var imgImport = $('<input type="file">').change(importImage);
				$('#tool_image').show().prepend(imgImport);
			}
